...
public:
	...
	const result_flag from_UTF16wstring_to_CP_ACP(const wchar_t* wchar_utf16_p, const size_t wchar_utf16_size, std::string& string_CP_ACP)
	{

		//КОРОЧЕ!!!!!!!!!! ИХ НЕНАДО ИСПОЛЬЗОВАТЬ, ТАК КАК ДАННЫЕ КОНВЕРТИРУЮТСЯ НЕКОРРЕКТНО. В ЭТОМ КОНКРЕТНОМ СЛУЧАЕ!! Из Национально кодировки конвертируется корректно, а наоборот в Национальную кодировку - некорректно. НЕИСПОЛЬЗОВАТь!




		const size_t size_before = string_CP_ACP.size();


		//---------------------------------------------------------Предварительный расчет необходимой длинны под результативный буффер:--------------------------------------------------------------------

		const int size_final_byte = WideCharToMultiByte(CP_ACP, 0, wchar_utf16_p, wchar_utf16_size, &string_CP_ACP[0], 0, NULL, NULL);  //5ым параметром я должен указать размер буффера под итоговое преобразование из UTF-16 в указанную кодировку, НО так как я не знаю какой размер будет занимать преобразованные символы, то Как видно из описания параметра 6, если вызвать функцию с 0-вым 6вым парметром, то функция вернет это кол-во байт для финального буффера.
		...
		//size_final_byte - теперь здесь итоговое кол-во байт, которое требуется для итогового буффера string_CP_ACP, чтобы туда преобразовать UTF-16 в CP_ACP.

		//-----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------



		try { string_CP_ACP.resize(size_before + size_final_byte); } catch (const std::bad_alloc& e) { error = e.what(); return  result_flag::bad_alloc; }



		//------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------

		int status = WideCharToMultiByte(CP_ACP, 0, wchar_utf16_p, wchar_utf16_size, &string_CP_ACP[size_before], size_final_byte, NULL, NULL); //Вызовем функцию WideCharToMultiByte - еще раз, но уже с size_final_byte:
	...
	const result_flag from_UTF8_to_CP_ACP(const char* char_UTF8_p, const size_t UTF8_size, std::string& string_CP_ACP)
	{

		//Так как функции MultiByteToWideChar и WideCharToMultiByte - преобразовывают между основной кодировкой UTF16 в дургие и из других в UTF16, то нет прямой функции, которая бы преобрзовала из UTF8 в однобайтовоую нацональную кодиовку.
		//ПОЭТОМУ сначала преобразуем из UTF8 в UTF16, а потом уже из UTF16 в "CP_ACP" в национальную кодирвку действующую на данный момент в системе.

		//КОРОЧЕ!!!!!!!!!! ИХ НЕНАДО ИСПОЛЬЗОВАТЬ, ТАК КАК ДАННЫЕ КОНВЕРТИРУЮТСЯ НЕКОРРЕКТНО. В ЭТОМ КОНКРЕТНОМ СЛУЧАЕ!! Из Национально кодировки конвертируется корректно, а наоборот в Национальную кодировку - некорректно. НЕИСПОЛЬЗОВАТь!


		//---------------------------------------------------------------Преобразуем из UTF8 в UTF16:Начало--------------------------------------------------
		std::wstring wstring_utf16_temp;

		result_flag result_ = from_UTF8_to_UTF16wstring(char_UTF8_p, UTF8_size, wstring_utf16_temp);

		if (result_ != result_flag::OK)
		{
			error = "from_UTF8_to_UTF16wstring: " + error;
		...
		//-----------------------------------------------------------------Преобразуем из UTF16 в CP_ACP:Начало--------------------------------------------------------------------

		result_ = from_UTF16wstring_to_CP_ACP(wstring_utf16_temp.c_str(), wstring_utf16_temp.size(), string_CP_ACP);
		...
		//-----------------------------------------------------------------Преобразуем из UTF16 в CP_ACP:Конец--------------------------------------------------------------------
